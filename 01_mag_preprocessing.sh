...
rm -rf ${output_1}KorefMicrobiomeShotgun3
...
do
  ...
  echo $bowtie2 -p 64 -1 ${output_1}${sample}/${sample}_1.fq.gz -2 ${output_1}${sample}/${sample}_2.fq.gz -x $hg38 --un-conc-gz ${sample}.host_removed -S ${output_2}${sample}/${sample}_mapped_and_unmapped.sam > ${scripts}${sample}_bowtie.sh
  echo mv ${sample}.host_removed.1 ${output_2}${sample} >> ${scripts}${sample}_bowtie.sh
  echo mv ${sample}.host_removed.2 ${output_2}${sample} >> ${scripts}${sample}_bowtie.sh
  echo mv ${output_2}${sample}/${sample}.host_removed.1 ${output_2}${sample}/${sample}.host_removed_R1.fastq.gz >> ${scripts}${sample}_bowtie.sh
  echo mv ${output_2}${sample}/${sample}.host_removed.2 ${output_2}${sample}/${sample}.host_removed_R2.fastq.gz >> ${scripts}${sample}_bowtie.sh 
  #echo $samtools view -bS ${output_2}${sample}/${sample}_mapped_and_unmapped.sam \> ${output_2}${sample}/${sample}_mapped_and_unmapped.bam >> ${scripts}${sample}_bowtie.sh
  #echo $samtools view -b -f 12 -F 256 ${output_2}${sample}/${sample}_mapped_and_unmapped.bam \> ${output_2}${sample}/${sample}_bothEndsUnmapped.bam >> ${scripts}${sample}_bowtie.sh
  #echo $samtools sort -n ${output_2}${sample}/${sample}_bothEndsUnmapped.bam ${output_2}${sample}/${sample}_bothEndsUnmapped_sorted.bam >> ${scripts}${sample}_bowtie.sh
