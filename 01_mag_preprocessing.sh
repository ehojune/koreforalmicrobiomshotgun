...
echo $fastp -i ${rawdata}${sample1}/${sample1}_1.fq.gz -I ${rawdata}${sample1}/${sample1}_2.fq.gz -o ${output_1}${sample1}/${sample1}_1.fq.gz -O ${output_1}${sample1}/${sample1}_2.fq.gz --html ${output_1}${sample1}/${sample1}.html --json ${output_1}${sample1}/${sample1}.json --report_title ${sample1}_after  --overrepresentation_analysis --trim_front1 18 --trim_tail1 3 --trim_front2 18 --trim_tail2 3 --cut_window_size 4 --cut_mean_quality 20 --n_base_limit 0 --thread 16 --cut_front --average_qual 20 > ${scripts}KorefMicrobiomeShotgun1_fastp_after.sh 
...
do
  ...
  echo $bowtie2 -p 64 -1 ${output_1}${sample}/${sample}_1.fq.gz -2 ${output_1}${sample}/${sample}_2.fq.gz -x $hg38 --un-conc-gz ${output_2}${sample}/${sample}_mapped_and_unmapped.sam > ${scripts}${sample}_bowtie.sh
